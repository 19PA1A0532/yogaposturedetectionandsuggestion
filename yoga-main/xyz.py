...
def compare_pose(image,angle_point,angle_user, angle_target ):
    ...
    if stage<=2 and stage >0:
        #print("FIGHTING!")
        cv2.putText(image, str("Almost There"), (170,30), cv2.FONT_HERSHEY_SIMPLEX, 1, [0,0,255], 2, cv2.LINE_AA)
        
        pass
    elif(stage>2):
        cv2.putText(image, str("Try Hard"), (170,30), cv2.FONT_HERSHEY_SIMPLEX, 1, [0,0,255], 2, cv2.LINE_AA)
...
def addFeatures(cap):
    # cap = cv2.VideoCapture(0)
    path = r'D:\VIT[college work]\4-2mainproject\yogaposturedetection\static\yoga_pose_1.jpg'
